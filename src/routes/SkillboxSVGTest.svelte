<script>
	...
	onMount( async () => {
        ...
        const circMesh = new THREE.Mesh(circ, circ_material);
        scene.add(circMesh);
       ...
       await loader.load(svgURL, function(svgGoPro) {
           ...
           //svg3D.scale.y *= -1;
            ...
            if (spec_svg === "golang") {
                svg3D.scale.set(1, 1, 1); // svg specifics
                p_x = 2.1;
                p_y = 122.0;
                p_z = 2.0;
            } else {
           ...
           svg3D.position.set(p_x, p_y, p_z);
            //svg3D.scale.set(spec_size, spec_size, spec_size); // car
           /*
*/
            // Lighting
           ...
           // circMesh.rotateOnAxis(yAxis, 0.01);
            ...
            renderer.setAnimationLoop(() => {
                //svg3D.rotation.x += 0.000;
                //svg3D.rotation.y += 0.005;
                //svg3D.rotateOnAxis(yAxis, 0.01);
                //svg3D.setRotationFromAxisAngle(yAxis, 0.51);
                //circMesh.rotation.x -= 0.001;
                ...
                //svg3D.translateOnAxis(yAxis, 90.0);
                /*
                if (svg3D.position.x <= 125 ) {
                    svg3D.position.x += 0.405;
                    //console.log(svg3D.position.x);
                } else {
                    svg3D.position.x = 0;
                }

                 */
                //svg3D.rotation.z += 0.000;
